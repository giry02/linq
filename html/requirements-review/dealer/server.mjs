...
async function replayApi(request, response, parsedUrl) {
  ...
  let apiPath = parsedUrl.pathname.slice(4) + parsedUrl.search;
  // The downloaded client was mechanically namespaced from /fleet to /dealer.
  // Translate every API path segment back to the captured production endpoint
  // so nested routes such as /common/auth/dealer/... replay their fixtures too.
  apiPath = apiPath.replace(/\/dealer(?=\/|$)/g, "/fleet");
...
async function serveStatic(response, pathname) {
  ...
  if (!relative || relative === "dealer") relative = "index.html";
  if (relative.startsWith("dealer/")) relative = relative.slice("dealer/".length);
...
const server = createServer(async (request, response) => {
  try {
    const parsedUrl = new URL(request.url, "http://localhost:3001");
    if (parsedUrl.pathname === "/fleet" || parsedUrl.pathname.startsWith("/fleet/")) {
      const dealerPath = `/dealer${parsedUrl.pathname.slice("/fleet".length)}${parsedUrl.search}`;
      response.writeHead(308, { location: dealerPath });
      response.end();
      return;
    }
...
server.listen(port, "127.0.0.1", () => {
  console.log(`Dealer mirror: http://localhost:${port}/dealer/ (offline)`);
});
