(() => {
  ...
  const modelForVehicle = value => {
    ...
    if (id.startsWith('FDB12')) return 'D30SE-9';
  ...
  const decorateUrl = value => {
    ...
    try {
      ...
      if (next.origin !== location.origin || !next.pathname.startsWith('/dealer/')) return value;
  ...
  function currentCompanyName(side) {
    return text(side?.querySelector('.page-side__title')) || '(\uc8fc)\ub450\uc0b0';
  }
  ...
  async function loadEquipment() {
    ...
    if (!equipmentPromise) {
      equipmentPromise = Promise.resolve([
        {id:'FDB12-000345', equipmentNumber:'FDB12-000345', model:'D30SE-9', power:'\uc5d4\uc9c4', companyId:'151', company:'(\uc8fc)\ub450\uc0b0'},
        {id:'FDB21-002254', equipmentNumber:'FDB21-002254', model:'D70S-9', power:'\uc5d4\uc9c4', companyId:'34317', company:'\ub450\uc0b0\ubc25\ucea3\ucf54\ub9ac\uc544 \uc8fc\uc2dd\ud68c\uc0ac'},
        {id:'FRA0V-001109', equipmentNumber:'FRA0V-001109', model:'BR25S-9', power:'\uc5d4\uc9c4', companyId:'33767', company:'\ub450\uc0b0\uc9c0\uac8c\ucc28 \uacbd\ub0a8\uc911\ubd80\ud310\ub9e4 \uc8fc\uc2dd\ud68c\uc0ac'},
      ]).then(rows => (equipmentRecords = rows));
    }
  ...
  function currentSubMenus() {
    ...
    return config.items
      .filter(([label]) => !(reviewScreen === 'operation-shock' && label === '\uc218\uc18c\ubc30\ud130\ub9ac'))
      .map(([sourceLabel, route]) => {
        const label = reviewScreen === 'engine-efficiency' && sourceLabel === '\uc5d4\uc9c4' ? '\uc5d4\uc9c4 \uc5f0\ube44' : sourceLabel;
        return {label, route, active: location.pathname.includes(route)};
      });
  }

  function routeScope() {
    const match = location.pathname.match(/\/(company|group|vehicle|equip)\/([^/?#]+)(?:\/([^/?#]+))?/);
    return match ? [match[1], match[2], match[3]].filter(Boolean).join('/') : 'company/151';
  }

  function menuUrl(route) {
    const language = location.pathname.match(/^\/dealer\/(ko|en)\//)?.[1] || 'ko';
    const prefix = `/dealer/${language}/page`;
    let pathname;
