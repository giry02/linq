(() => {
  ...
  const reviewGuide = {
    ...
    'lithium-battery': [
      {ppt:'PPT 7p',id:'R-BAT-001',request:'리튬배터리 상세 화면 하단의 에러 정보 영역을 삭제한다. 에러 정보는 서비스의 차량 에러 화면에 통합하여 별도로 표시하지 않는다.',implementation:'운영의 FBA32_224250383 조회값(잔여 배터리 84%, 상태 정상, 작업 가능 7시간 6분, 성능상태 100%)과 8월 충·방전 및 온도 범위를 로컬 화면에 반영하고, 하단 에러 정보 섹션만 제거했다. 추가 수정으로 본문 차량별 선택은 제거하고 상단 조회 대상에서 FBA32_224250383 차량을 선택한 상태로 고정했다.',location:'리튬배터리 상세 요약·충방전·온도 그래프 및 화면 하단',status:'UI 적용 / 운영 조회값 반영 / 추가 수정: 상단 특정 차량 선택 / 중복 오류 API 호출 제거 필요'},
  ...
  const requirementCallouts = {
    ...
    'lithium-battery': [
      callout(1,'R-BAT-001','PPT 7p','운영 배터리 데이터 반영·에러 정보 제거','리튬배터리 상세 화면 하단의 에러 정보 영역을 삭제한다. 에러 정보는 서비스의 차량 에러 화면에 통합하여 별도로 표시하지 않는다.','운영의 FBA32_224250383 조회값과 8월 충·방전 및 온도 범위를 로컬 상세 화면에 반영하고 하단 에러 정보 섹션만 제거했다. 추가 수정으로 본문 차량별 선택을 제거하고 상단 조회 대상에서 해당 차량을 선택한 상태로 고정했다.','.content-body','리튬배터리 상세 요약·충방전·온도 그래프 및 화면 하단','UI 적용 / 운영 조회값 반영 / 추가 수정: 상단 특정 차량 선택 / 중복 오류 API 호출 제거 필요'),
