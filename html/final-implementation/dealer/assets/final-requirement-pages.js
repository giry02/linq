(() => {
  if (window.__linqFinalRequirementPagesMounted) return;
  window.__linqFinalRequirementPagesMounted = true;

  const finalMode = window.LINQ_FINAL_IMPLEMENTATION === true;
  ...
  const items = finalMode ? [] : routeItems();
  if (!items.length && !finalMode) return;
  const entries = [];
  let activeItem = null;
  ...
  if (finalMode) guide.hidden = true;
  ...
  function configurePeriodFilter(content) {
    ...
    if (inputs.length >= 2) {
      ...
      if (host && !host.querySelector('.linq-review-period-help')) {
        ...
        help.textContent = '사용자설정은 시작일과 종료일을 직접 선택합니다.';
